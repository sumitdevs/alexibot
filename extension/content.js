...
(function() {
  ...
  function setupTooltipListeners() {
    ...
    const saveIcon = state.tooltip.querySelector('#saveicon');
  ...
  function extractContext(selection) {
    try {
      ...
      const end = Math.min(fullText.length, selectionIndex + selectedText.length + CONFIG.contextRadius);
  ...
  function handleSpeak() {
    ...
    if (audioUrl) {
      ...
      const utterance = new SpeechSynthesisUtterance(state.currentSelection.word);
      utterance.rate = 0.9;
